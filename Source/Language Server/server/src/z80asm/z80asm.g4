...
//options { superClass=GrammarProfile; }
...
//directive_file: ({this.zmac}? ZMAC_DIRECTIVEfile | {this.z88dk}? Z88DK_DIRECTIVEfile) STRING;
directive_file: (ZMAC_DIRECTIVEfile | Z88DK_DIRECTIVEfile) STRING;
...
//directive_symbollist: ({this.zmac}? ZMAC_DIRECTIVEsymbollist | {this.z88dk}? Z88DK_DIRECTIVEsymbollist) symbol (COMMA symbol)*;
directive_symbollist: (ZMAC_DIRECTIVEsymbollist | Z88DK_DIRECTIVEsymbollist) symbol (COMMA symbol)*;
//directive_assign: {this.z88dk}? (DEFC symbol EQUALS expression) | {this.zmac}? (symbol EQU expression);
directive_assign: (DEFC symbol EQUALS expression) | (symbol EQU expression);
//directive_block: ({this.zmac}? ZMAC_DIRECTIVEblock | {this.z88dk}? Z88DK_DIRECTIVEblock) (expression | STRING) (COMMA (expression | STRING))*;
directive_block: (ZMAC_DIRECTIVEblock | Z88DK_DIRECTIVEblock) (expression | STRING) (COMMA (expression | STRING))*;
//directive_defs: ({this.zmac}? ZMAC_DIRECTIVEskip | {this.z88dk}? Z88DK_DIRECTIVEskip) expression (COMMA expression)?;
directive_defs: (ZMAC_DIRECTIVEskip | Z88DK_DIRECTIVEskip) expression (COMMA expression)?;
...
//label: {this.z88dk}? z88dk_label | {this.zmac}? zmac_label;
label: z88dk_label;
z88dk_label: (DOT SYMBOL) | (SYMBOL COLON);
zmac_label: SYMBOL COLON? COLON?;
//zmac_label: (ZMAC_LABEL_ex)+ COLON? COLON?;
//ZMAC_LABEL_ex: ALPHANUMEX | DOLLAR | DOT | QUESTION | AT;
...
// @code
//number_bin: {this.z88dk}? Z88DK_NUMBERbin | NUMBERbin;
...
// @code
//number_hex: {this.zmac}? ZMAC_NUMBERhex | {this.z88dk}? Z88DK_NUMBERhex | INSTRUCTIONrstvectorhex;
number_hex: Z88DK_NUMBERhex | INSTRUCTIONrstvectorhex;
// @code
...
INSTRUCTIONrstvectorhex: DOLLAR INSTRUCTIONrstvector | INSTRUCTIONrstvector H;
INSTRUCTIONrstvector: [0-3][08];

Z88DK_NUMBERbin: ('@' DIGIT2+);
NUMBERbin: (DIGIT2+ 'b');
NUMBERoct: DIGIT8+ ('o' | 'q');
NUMBERdec: DIGIT10+ 'd'?;
ZMAC_NUMBERhex: (DIGIT16+ 'h') | ('0x' DIGIT16+);
Z88DK_NUMBERhex: (DIGIT16+ H) | (DOLLAR DIGIT16+) | ('0x' DIGIT16+);
